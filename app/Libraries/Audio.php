...
class Audio
{
    ...
    public static function parseFrameHeader($fourbytes)
    {
        ...
        $newvar = 0;
        if(empty($sample_rate)) { print_r($sample_rate); }
        if ($layer == 1)
            $newvar = intval(((12 * $bitrate * 1000 / $sample_rate) + $padding_bit) * 4);
        else //layer 2, 3
            $newvar = intval(((144 * $bitrate * 1000) / $sample_rate) + $padding_bit);
        //var_dump($newvar); die();

        $info['Framesize'] = $newvar; //self::framesize($layer, $bitrate, $sample_rate, $padding_bit);
        //die();
        //var_dump($samples);
    ...
    private static function framesize($layer, $bitrate,$sample_rate,$padding_bit)
    {
        //var_dump($layer);
        //var_dump($bitrate);
        //var_dump($sample_rate);
        //var_dump($padding_bit);
        //var_dump(intval(((144 * $bitrate*1000)/$sample_rate) + $padding_bit));
        //die();
        if ($layer==1)
