...
Route::get('/checkduration', function () {
    //$path = public_path().'/uploads/files/1614805961_mehanicheskiy-ded-moroz-01.mp3';
    $path = public_path().'/uploads/files/1623081442_emil-iz-lyennebergi-1.mp3';
    //print_r($path); die();
    ...
    //die();
    print_r($audio->formatTime($duration)); die();
*/
/*
    $item = 0;
    //duration block
    $file_audios = \App\File::where('type', 1)->whereHas('items', function($q) use  ($item){$q->where('item_id', '=', 123);})->get();
...
Route::post('/checkingpost', function () {
    //$file = "http://192.168.56.101/time/public/uploads/files/1613424698_patrul-1.mp3";
    /*$file = public_path()."/uploads/files/1613424698_patrul-1.mp3";
    $audio = new \App\Libraries\Audio($file);
    $duration1 = $audio->getDuration();
    dd($audio->formatTime($duration1));*/
    //$data = request()->all();
    //$item = \App\FilesHistory::find(1);
    //$items = \App\Item::where('id', 20)->with('files')->firstOrFail();
    //return response()->json($items);
    $item = \App\Item::find(64);
    $tag = new \App\Tag();
    //$tag->name = 'Petson';
    //$tag->save();
...
// save image in desired format
    ...
    //$items = \App\Item::where('id', 20)->with('files')->firstOrFail();
    //return response()->json($items);
    //$item = \App\Item::find(67);
    //$tag = new \App\Tag();
    //$tag->name = 'Petson';
    //$tag->save();
    //$item->tags()->attach($tag->id);
    //echo 1;
    //$duration = \App\File::where('type', 1)->whereHas('items', function($q){$q->where('id', '=', 67);})->get();
    //$duration = \App\File::where('type', 1)->whereHas('items', function($q) use  ($item){$q->where('item_id', '=', $item->id);})->get();
    //return $duration;
})->name('checkingget');
...
Route::post('/item/save', function () {
    ...
    //return response()->json('end');//, $file);


    //$car->options()->sync($request->input('options'));
    /*
    $item = \App\Item::where('id', $item->id)->with('files')->with("tags")->firstOrFail();
